# -*- coding: utf-8 -*-
"""
Created on Tue May 26 15:30:05 2026
...
key_list = ['Ischemic heart disease', 'Severe coronary stenosis diseases', 'Metabolite diseases', 'Coronary artery disease in diabetes', 'Vascular endothelium diseases', 'Coronary heart disease', 'Cholesterol lipid metabolism diseases' , 'Immune response inflammation diseases' , 'Hypertension']


chrom = ['chr' + str(x) for x in range(1 , 23)] + ['chrX']


CAD_realated_snps = [] ; Final_Dict = {}

for k in key_list:
    print (k)
    s_classify = data_new_classify[k]
    Final_Dict[k] = []
    for g in chrom:
        # print(g)
        tmp_select = s_classify[s_classify['CHR_ID'] == g.lstrip('chr')]
        ...
        for i in tmp_select.index:
            ...
            if len(overlap) > 0:
                ...
                for j in overlap.index:
                    ...
                    tmp = list(set(tmp))
            Final_Dict[k].extend(tmp)
            Final_Dict[k].append((g , pos , rs))
        
            CAD_realated_snps.extend(tmp)
            CAD_realated_snps.append((g , pos , rs))
    Final_Dict[k] = pd.DataFrame(Final_Dict[k] , columns=['chr' , 'pos' , 'SNPs'])
    Final_Dict[k] = Final_Dict[k].drop_duplicates(subset=['SNPs'] , keep = 'first')
...
CAD_realated_snps.to_csv('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\Hapmap\\CAD_related_SNPs_LD0.8_all.bed' , header = None , index = None , sep = '\t')
...
CAD_realated_snps['start'] = CAD_realated_snps['pos'] - 200
CAD_realated_snps['end'] = CAD_realated_snps['pos'] + 200        

CAD_realated_snps[['chr' , 'start' , 'end']].to_csv('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\Hapmap\\CAD_related_SNPs_LD0.8_all_+-200bp.bed' , header = None , index = None , sep = '\t')         
...
for c in key_list:
    ...
    tmp.to_csv('D:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\CAD_related_' + c + 'SNPs_LD0.99_all.bed' , sep = '\t' , index=False)
    
    out = open('D:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\CAD_related_' + c + 'SNPs_LD0.99+-1bp_all.bed' , 'w')
    for i in tmp.index:
...
snp_num = ['snp_' + str(x) for x in range(len(tmp))]
tmp.insert(0 , 'SNP' , snp_num , allow_duplicates=False)
tmp.to_csv('D:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\CAD_related_SNPs_LD0.99_all.bed' , sep = '\t' , index=False)

out = open('D:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\CAD_related_SNPs_LD0.99+-1bp_all.bed' , 'w')
for i in tmp.index:
    out.writelines('\t'.join(['chr' + tmp.loc[i].CHR_ID , str(tmp.loc[i].CHR_POS - 1) , str(tmp.loc[i].CHR_POS + 1)]) + '\n')
out.close()
...
disease_name = ['高血压' , '血管内皮疾病' , '糖尿病中的冠状动脉疾病' , '胆固醇脂代谢疾病' , '冠心病' , '缺血性心脏病' , '严重冠状动脉狭窄疾病' ,
                '代谢疾病' , '免疫反应性炎症疾病']
english_name = ['Hypertension'  , 'Vascular endothelium diseases' , 'Coronary artery disease in diabetes' , 'Cholesterol lipid metabolism diseases' , 
                'Coronary heart disease' , 'Ischemic heart disease' ,  'Severe coronary stenosis diseases' , 'Metabolite diseases', 
                'Immune response inflammation diseases']
...
run_Plot(fig , 'H:\\work\\Postdoctoral\\GWAS疾病位点检测\\Plot\\CADe_related_SNP_LD0.9_Numbers_barplot_all_SNP_new_4.pdf')  
