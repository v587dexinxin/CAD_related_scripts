...
###############LSS_VS_OSS
outdir <- "/scratch/2026-05-18/bio-shenw/Cardiovascular_disease_STARR-seq/HUVEC_Cardiovascular_disease_moudle/HiRPC/one-dimension_new/mapping_new/DiffBind/Diffbind_DEseq2"
sample_sheet <- file.path(outdir, "LS_VS_OS_linux_1.csv")
...
write.table(
  res_df,
  file = file.path(outdir, "HUVEC_LS_VS_OS_deseq2_RLE_BACKGROUND_minOverlap1_all.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
outdir <- "/scratch/2026-05-11/bio-shenw/Cardiovascular_disease_STARR-seq/HUVEC_Cardiovascular_disease_moudle/HiRPC/one-dimension_new/mapping_new/DiffBind/Diffbind_DEseq2/WT_VS_OS"
sample_sheet <- file.path(outdir, "WT_VS_OS_linux_1.csv")
...
write.table(
  res_df,
  file = file.path(outdir, "HUVEC_WT_VS_OS_deseq2_RLE_BACKGROUND_minOverlap1_all.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  WT_specific,
  file = file.path(outdir, "HUVEC_WT_VS_OS_DESeq2_WT_specific_peaks_q0.05_fc0.5.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  OSS_specific,
  file = file.path(outdir, "HUVEC_WT_VS_OS_DESeq2_OSS_specific_peaks_q0.05_fc0.5.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
outdir <- "/scratch/2026-05-18/bio-shenw/Cardiovascular_disease_STARR-seq/HUVEC_Cardiovascular_disease_moudle/HiRPC/one-dimension_new/mapping_new/DiffBind/Diffbind_DEseq2/WT_VS_LS"
sample_sheet <- file.path(outdir, "WT_VS_LS_linux_1.csv")
...
write.table(
  count_df,
  file = file.path(outdir, "HUVEC_WT_VS_LS_DiffBind_raw_counts_for_DESeq2.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
# -----------------------------
# 6. 提取样本 count 列
# 根据你的样本名包含 WT / LS / WT / LSS 来识别
# -----------------------------
sample_cols <- grep("WT|LSS|WT|LS", colnames(count_df), value = TRUE)
...
condition <- factor(condition, levels = c("WT", "LSS"))
...
# LS vs WT
res_LSS_vs_WT <- results(
  dds,
  contrast = c("condition", "LSS", "WT")
)

# -----------------------------
# 11. 整理结果，并合并 peak 坐标
# -----------------------------
res_df <- as.data.frame(res_LSS_vs_WT)
res_df$peak_id <- rownames(res_df)
...
write.table(
  res_df,
  file = file.path(outdir, "HUVEC_WT_VS_LS_deseq2_RLE_BACKGROUND_minOverlap1_all.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  WT_specific,
  file = file.path(outdir, "HUVEC_WT_VS_LS_DESeq2_WT_specific_peaks_q0.05_fc0.5.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  LSS_specific,
  file = file.path(outdir, "HUVEC_WT_VS_LS_DESeq2_LSS_specific_peaks_q0.05_fc0.5.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
...
write.table(
  no_sign,
  file = file.path(outdir, "HUVEC_WT_VS_LS_DESeq2_common_peaks_q0.05_fc0.5.csv"),
  sep = ",",
  quote = FALSE,
  row.names = FALSE
)
