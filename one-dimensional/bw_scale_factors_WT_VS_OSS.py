# -*- coding: utf-8 -*-
"""
Created on Sun May 17 18:25:06 2026
...
deseq2_pos = pd.read_csv('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\HiRPC\one-dimensional_new\\DiffBind\\Diffbind_DEseq2\\WT_VS_OS\\HUVEC_WT_VS_OS_deseq2_RLE_BACKGROUND_minOverlap1_all.csv' , header=0)

deseq2_pos.columns = ['peak_id', 'chr', 'start', 'end', 'baseMean', 'log2FoldChange', 'lfcSE', 'stat', 'pvalue', 'padj', 'direction']
deseq2_pos["start"] = deseq2_pos["start"].astype(int)
no_signific = deseq2_pos[deseq2_pos['padj'] > 0.05]
no_diff = no_signific[no_signific['log2FoldChange'].abs() <= 0.5]
...
no_diff_h.to_csv('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\HiRPC\\one-dimensional_new\\signals\\all_reps\\stable_housekeeping_rescaled_bw\\WT_OSS_stable_housekeeping_peaks.bed' , header = None , index = None , sep = '\t')
...
signal_file = "H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\HiRPC\\one-dimensional_new\\signals\\all_reps\\stable_housekeeping_rescaled_bw\\WT_VS_OS_stable_peaks_bw_signal.tab"
...
scale_factors.to_csv(
    "H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\HiRPC\\one-dimensional_new\\signals\\all_reps\\stable_housekeeping_rescaled_bw\\WT_OSS_stable_peaks_trimmedMean_scale_factors.txt",
    sep="\t",
    header=["scale_factor"]
)


####results: LSS_scales factor: 0.605
