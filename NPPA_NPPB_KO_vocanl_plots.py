# -*- coding: utf-8 -*-
"""
Created on Tue Dec 23 21:46:59 2025
...
def Volcano_Gene_Plot(fil,title, s1 , s2 , gene_names , qmax , fmax , fc = 0.5 , q = 0.01):
    ...
    Gene_type = np.dtype({'names':['Gene_name' , 'FC' , 'p'],
    ...
    NC_mask = (Gene['FC']> NC_bound) & (Gene['p'] > -math.log10(q))
    NC_Genes = Gene[NC_mask]
    
    si_mask = (Gene['FC'] < si_bound) & (Gene['p'] > -math.log10(q))
    ...
    ax.scatter(NC_Genes['FC'],NC_Genes['p'], s= 10, c = 'red')
    ax.scatter(si_Genes['FC'],si_Genes['p'], s= 10, c= 'blue')
    ax.scatter(Non_Genes['FC'],Non_Genes['p'], s= 10, c = 'gray')
...
#####WT_VS_NPPA_NPPB_KO13

genes = pd.read_csv('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\Confirmation_Experiment\\NPPA_NPPB_peak2_敲除\\RNA_seq\\DEseq2\\HCT116_WT_VS_NP13_KO_deseq2.csv' , header = 0)
gene_names = ['NPPA' , 'NPPA-AS1' , 'NPPB']


pp1 = PdfPages('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\Confirmation_Experiment\\NPPA_NPPB_peak2_敲除\\RNA_seq\\DEseq2\\HCT116_WT_VS_NPPA_B_KO13_DEGs_scatter_p0.05_fc0.5_1.pdf')
fig = Volcano_Gene_Plot('H:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\CAD\\first_6000\\Confirmation_Experiment\\NPPA_NPPB_peak2_敲除\\RNA_seq\\DEseq2\\HCT116_WT_VS_NP13_KO_deseq2.csv','WT_VS_KO13_DEGs', 'WT' , 'KO13' , gene_names , 40 , 8 , fc = 1, q = 0.05)
