...
for i in range(len(data)):
    for j in data.loc[i]:
        j = str(j)
        a = j.lower()
        if ('carditis' in a) or ('stroke' in a) or ('heart' in a) or ('cardiovascular' in a) or ('coronary artery' in a) or ('cardiac' in a) or ('arrhythmia' in a) or ('hypertension' in a) or ('cardiomyopathy' in a)  or ('atherosclerosis' in a) :
            # print (i)
            number.append(i)
            break
...
data_new = data_new[data_new['INTERGENIC'] == 1]
...
outfil = open('D:\\work\\Postdoctoral\\GWAS疾病位点检测\\results\\Cardiovascular_disease_related_SNP_intergenetic_+-1Kb.bed' , 'w')
for g in keys:
    for i in data_2[g]:
