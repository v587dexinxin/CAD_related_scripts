# -*- coding: utf-8 -*-
"""
Created on Wed May 10 20:56:40 2023
...
'''
CAD related disease classify(首次选择4000个tag SNP)
'''
...
n = 0
for key in key_list:
    v = data_new_classify[key]
    for i in v.index:
        g = v.loc[i].CHR_ID
        pos = int(v.loc[i].CHR_POS)
        rs = v.loc[i].SNPS
        if (g , pos , rs) not in UniqueSet:
                n += 1
                Final_Dict[key].append((g , pos , rs))
                UniqueSet.add((g , pos , rs))
